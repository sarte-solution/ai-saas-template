...
---
AI SaaS Template features a robust authentication system built with AI SaaSr Auth, supporting multiple OAuth providers, role-based access control, and secure session management.
...
```typescript
import { AI SaaSrAuth } from "AI SaaSr-auth"
import { drizzleAdapter } from "AI SaaSr-auth/adapters/drizzle"
...
export const auth = AI SaaSrAuth({
...
```typescript
import { createAuthClient } from "AI SaaSr-auth/react"
import { inferAdditionalFields } from "AI SaaSr-auth/client/plugins"
...
```tsx
// src/components/blocks/sign-up/sign-up-form.tsx
"use client"
...
describe('Authentication API', () => {
  it('should create user account', async () => {
    const response = await testClient.post('/api/auth/sign-up', {
  ...
  it('should sign in user', async () => {
    const response = await testClient.post('/api/auth/sign-in', {
      email: 'test@example.com',
      password: 'password123'
    })
...
1. **OAuth Callback Errors**
   ...
   - Ensure AI SaaSR_AUTH_URL matches your domain
...
```bash
# Check user sessions
psql -d AI SaaSrsaas -c "SELECT * FROM sessions WHERE userId = 'user-id';"

# Check user roles
psql -d AI SaaSrsaas -c "SELECT id, email, role FROM users;"
